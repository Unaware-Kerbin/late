...
- **High** — token or vault disclosure, unauthenticated RPC, cloud chat without opt-in
- **Medium** — path jail bypass with token, supply-chain in recommended Hub ids
- **Low** — documentation mismatch, unauthenticated `/health` feature list
...
**Evidence:** `audit.jsonl` on the affected machine; GitHub issue or private report; CI logs. There is no 24/7 on-call.
