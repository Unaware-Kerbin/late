...
3. **Local inference first.** Cloud agent backends (Cursor and public-internet chat URLs, including api.anthropic.com, Gemini, and public Azure) are off until the operator turns on **Cloud AI** in Settings. That toggle is audited. Session text may then leave the operator’s computer. OpenAI-compatible URLs and native Anthropic / Gemini / Azure endpoints on loopback or a private network (RFC1918, IPv6 ULA, `.internal` / `.local`, or `private_inference_hosts`) are allowed with Cloud AI off; audit `egress` is `loopback` or `private`.
4. **Hugging Face is for weights.** GGUF download and Ollama Pull of Hub ids are operator-initiated. They are not chat APIs.
5. **Sidecar gates.** Dual-gate is sidecar-only. On network OS, agent `propose_command` runs the vendor permit list, then the operator must Approve. Linux CLI has no permit list (Approve every command). `propose_api_get` is operator click only (no vendor permit list). Always-allow (non-Linux) still re-runs the permit check. Daemon `session.input` is ungated for a client that already has the token. The agent cannot SCP/SFTP or call `inference.download`.
