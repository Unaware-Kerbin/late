...
|---|---|---|---|---|
| R1 | Same-OS-user process reads `sidecar.token` and calls RPC | H | H | Accept for single-user desktop. Org: lock screen, no shared logins. Mitigate: 0600 token, loopback Host check. |
| R2 | Cloud chat sends scrollback off-box without operator intent | M | H | Mitigate: `cloud_chat_enabled` default false; sidecar refuses Cursor and non-loopback bases until opt-in; audit toggle and `/chat` egress. |
| R3 | Vaults are plaintext JSON (chmod 0600 only) | H | H | Accept this pass (no AES/age yet). Org: FDE. Later product: AEAD. |
...
| R8 | Electron `--no-sandbox` on Linux | H | M | Accept this pass (GPU/Wayland). Documented residual. |
